...
describe('scenario:setFeeTo', () => {
  ...
  beforeEach('deploy hubdao', async () => {
    factory = await deployContract(wallet, HubdaoFactory, [timelock.address])
  })
  ...
  it('setFeeTo', async () => {
    ...
    const description = 'Set feeTo on the HubdaoFactory to the timelock address.'

    // activate balances
    await hd.delegate(wallet.address)
