...
describe('GovernorAlpha', () => {
  ...
  it('hd', async () => {
    const balance = await hd.balanceOf(wallet.address)
    const totalSupply = await hd.totalSupply()
  ...
  it('governor', async () => {
    ...
    const hdFromGovernor = await governorAlpha.hd()
    expect(hdFromGovernor).to.be.eq(hd.address)
  })
})
