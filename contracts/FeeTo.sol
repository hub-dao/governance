...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IHubdaoPair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IHubdaoPair(pair).token1()];
        ...
        if (
            ...
            value = IHubdaoPair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IHubdaoPair(pair).transfer(pair, value));
                IHubdaoPair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IHubdaoPair(pair).transfer(feeRecipient, value));
            }
        }
    }
}
